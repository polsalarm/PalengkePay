  ...
  return (
    <div ref={ref} className="relative">
      <button
        ...
        className="flex items-center gap-2 bg-white border border-slate-200 hover:border-teal-300 text-slate-700 text-sm font-medium px-3 py-1.5 rounded-full transition-all shadow-sm"
      >
        <span className="w-2 h-2 rounded-full bg-green-500 shrink-0" />
        <span className="font-mono text-xs">{truncateAddress(address!)}</span>
        {balance !== null && (
          <span className="text-teal-700 font-semibold">{balance} XLM</span>
        )}
      </button>
      ...
      {open && (
        <div className="absolute right-0 top-full mt-2 w-72 bg-white rounded-xl shadow-lg border border-slate-200 p-4 z-50">
          ...
          <div className="bg-slate-50 rounded-lg p-3 mb-3 flex items-center justify-between gap-2">
            ...
            <button
              onClick={copyAddress}
              className="shrink-0 text-slate-400 hover:text-teal-600 transition-colors"
              title="Copy address"
            >
              <Copy size={14} />
            </button>
          ...
          <button
            ...
            className="flex items-center gap-2 w-full text-sm text-red-600 hover:bg-red-50 px-3 py-2 rounded-lg transition-colors"
          >
            <LogOut size={14} />
            Disconnect
          </button>
